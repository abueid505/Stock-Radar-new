...
def fetch_real_stock_data() -> tuple[dict, str]:
    ...
    try:
        tickers = yf.Tickers(" ".join(SCANNER_SYMBOLS))
        new_cache = {}
        errors = []
        ...
        for symbol in SCANNER_SYMBOLS:
            try:
                ...
                previous_close = float(info.previous_close) if hasattr(info, 'previous_close') and info.previous_close else None
...
def generate_stock_signal(symbol: str, real_data: dict) -> StockSignal:
    """Generate a stock signal using real price data with analyst targets"""
    ...
    buy_price = round(current_price * random.uniform(0.95, 0.98), 2)
    sell_price = round(current_price * random.uniform(1.05, 1.12), 2)
    potential_profit = round(((sell_price - buy_price) / buy_price) * 100, 2)
    ...
    simulated_prices = [current_price * (1 + random.uniform(-0.02, 0.02)) for _ in range(20)]
    simulated_prices.append(current_price)
    rsi = calculate_rsi(simulated_prices)
    
    forecast_up = sell_price > current_price
    ...
    return StockSignal(
        ...
        num_shares=random.randint(10, 100) * 10,
...
async def get_stock_detail(symbol: str):
    ...
    signal = generate_stock_signal(symbol, real_data)
    base_price = real_data[symbol]["current_price"]
    
    return {
        "signal": signal.model_dump(),
        "chart_1d": [c.model_dump() for c in generate_chart_data(base_price, "1d")],
        "chart_1w": [c.model_dump() for c in generate_chart_data(base_price, "1w")],
        "chart_1m": [c.model_dump() for c in generate_chart_data(base_price, "1m")],
        "warning": error_msg if error_msg else None
    }
...
async def check_alerts():
    ...
    for symbol in STOCK_SYMBOLS:
        ...
        # Simulate target hit (10% chance for demo)
        if random.random() < 0.1 and signal.symbol not in triggered_alerts:
            message = (
                f"Target Price Hit!\n\n"
                f"Symbol: {signal.symbol}\n"
                f"Company: {signal.company_name}\n"
                f"Current Price: ${signal.current_price}\n"
                f"Target Price: ${signal.recommended_sell_price}\n"
                f"Profit: {signal.potential_profit_percent}%"
            )
...
def fetch_single_stock(symbol: str) -> dict:
    """Fetch data for a single stock on-demand with 5-minute caching."""
    ...
    try:
        ...
        # Get full info for analyst data and company name
        full_info = ticker.info
...
def generate_manual_stock_signal(symbol: str, stock_data: dict) -> StockSignal:
    """Generate a stock signal for a manually searched stock."""
    current_price = stock_data["current_price"]
    closing_price = stock_data["previous_close"]
    ...
    return StockSignal(
        ...
        num_shares=random.randint(10, 100) * 10,
        potential_profit_percent=potential_profit,
        reason=f"Technical analysis suggests potential upside for {symbol}",
        latest_news=[f"Real-time data for {symbol}", "Analyst consensus available"],
        analyst_target_price=analyst_target,
        analyst_recommendation=analyst_rec,
...
async def get_manual_stock_detail(symbol: str):
    """Get detailed data for a manually searched stock including chart data."""
    symbol = symbol.upper().strip()
    ...
    if symbol in real_data and symbol in STOCK_METADATA:
        ...
        return StockDetail(
            signal=signal,
            chart_1d=generate_chart_data(signal.current_price, "1d"),
            chart_1w=generate_chart_data(signal.current_price, "1w"),
            chart_1m=generate_chart_data(signal.current_price, "1m")
        )
